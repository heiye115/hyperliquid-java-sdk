...
<dependency>
    ...
    <version>0.2.22</version> <!-- 建议替换为最新版本 -->
</dependency>
```
...
```gradle
implementation 'io.github.heiye115:hyperliquid-java-sdk:0.2.22' // 建议替换为最新版本
```

## ⚡ 5分钟快速体验

这一节分为两个渐进步骤：先只读查询，再在需要交易时添加钱包配置。
...
public class QuickStartReadOnly {
    ...
    public static void main(String[] args) {
        ...
        LOGGER.info("最优卖价: {}", l2Book.getLevels().get(0).get(0));
        LOGGER.info("最优买价: {}", l2Book.getLevels().get(1).get(0));
    }
}
```
...
1. 客户端默认连接主网。
2. 如果需要测试网，请在 Builder 中调用 `.testNetUrl()`。
3. 准备私钥（主钱包私钥或 API 钱包私钥）。
4. 通过环境变量管理私钥。

```bash
export HYPERLIQUID_PRIVATE_KEY="0x您的私钥"
```
...
public class QuickStartTrade {
    ...
    public static void main(String[] args) {
        ...
        LOGGER.info("下单响应: {}", JSONUtil.writeValueAsString(order));
    }
}
```

更多完整场景请查看 `examples/` 目录中的可运行示例。
...
HyperliquidClient client = HyperliquidClient.builder()
        // 网络选择
        // 默认主网，无需额外配置
        // .testNetUrl() // 如需测试网请启用此行
        // 或 .baseUrl("https://api.hyperliquid.xyz")
        ...
        .addApiWallet("bot-2", "0x您的主钱包地址2", "0x您的API私钥2")
...
```java
ClearinghouseState userState = info.userState("0x您的地址");
LOGGER.info("可提现金额: {}", userState.getWithdrawable());
```
...
```java
UpdateLeverage leverageResponse = exchange.updateLeverage("ETH", false, 20); // 20倍杠杆，逐仓模式
```
...
try {
    ...
    if (e instanceof HypeError.ClientHypeError ce) {
        LOGGER.error("客户端错误 (status={}): {}", ce.getStatusCode(), ce.getMessage());
    } else if (e instanceof HypeError.ServerHypeError se) {
        LOGGER.error("服务端错误 (status={}): {}", se.getStatusCode(), se.getMessage());
    } else {
        LOGGER.error("SDK 错误: {}", e.getMessage());
    }
}
```
